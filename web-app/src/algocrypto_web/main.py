...
from algocrypto_web.auth import (
    LoginRequest,
    TokenResponse,
    create_access_token,
    require_user,
...
from algocrypto_web.paper_account import IST_TRADE_DATE, ensure_paper_account
...
class Settings(BaseSettings):
    ...
    database_url: str = "postgresql://algocrypto:algocrypto@localhost:5432/algocrypto"
    trading_engine_url: str = "http://127.0.0.1:8001"


settings = Settings()
app = FastAPI(title="Algo-Crypto Web", version="0.3.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def market_summary(_user: User) -> dict[str, Any]:
    ...
    row = await pool().fetchrow(
        f"""
        SELECT
            COALESCE(starting_capital, 250) AS starting_capital,
            COALESCE(available_capital, 250) AS available_capital,
    ...
    if row:
        # Engine is source of truth when reachable. Only fill missing keys from DB.
        data.setdefault("starting_capital", float(row["starting_capital"]))
        data.setdefault("available_capital", float(row["available_capital"]))
        data.setdefault("deployed_capital", float(row["deployed_capital"]))
        data.setdefault("today_pnl", float(row["today_pnl"]))
        data.setdefault("trade_count", int(row["trade_count"]))

        open_count = int(data.get("open_position_count") or 0)
        has_open = bool(data.get("has_open_position")) or open_count > 0
        if not has_open:
            # Flat book → used margin must be 0; free cash = starting + realized P&L.
            start = float(data["starting_capital"])
            realized = float(data["today_pnl"])
            data["deployed_capital"] = 0.0
            data["used_margin"] = 0.0
            data["available_capital"] = start + realized
        else:
            data["used_margin"] = float(
                data.get("used_margin", data.get("deployed_capital", row["deployed_capital"]))
            )
    else:
        data.setdefault("starting_capital", 250.0)
        data.setdefault("available_capital", 250.0)
...
async def trade_blotter(_user: User, limit: int = 20) -> dict[str, Any]:
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            resp = await client.get(
