...
function resolveRawItems(
  ...
  // Single-underlying crypto feed (no commodities[] on watchlist).
  if (watchlist?.items?.length && watchlist.underlying === activeCommodity) {
    return watchlist.items;
  }
  return [];
}
...
export function useCommodityChain(
  ...
  const selected =
    commodities.find((c) => c.underlying === activeCommodity) ??
    (watchlist && watchlist.underlying === activeCommodity
      ? {
          underlying: watchlist.underlying,
          display_name: watchlist.underlying,
          spot_ltp: watchlist.spot_ltp,
          atm_strike: watchlist.atm_strike,
          strike_step: watchlist.strike_step,
          items: watchlist.items,
        }
      : undefined);

  const rawItems = useMemo(
    () => resolveRawItems(selected, watchlist, activeCommodity),
    [selected, watchlist, activeCommodity],
  );
