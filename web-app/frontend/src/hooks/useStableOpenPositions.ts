...
function mergePosition(
  ...
  return {
    ...prev,
    ...next,
    entry_price: next.entry_price ?? prev.entry_price,
...
export function useStableOpenPositions(
  ...
  const live = Boolean(watchlist?.feed_mode?.includes("websocket") || watchlist?.feed_mode === "live");
