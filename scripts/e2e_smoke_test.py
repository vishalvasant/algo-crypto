#!/usr/bin/env python3
"""End-to-end smoke test for Algo-Crypto stack."""
...
async def main() -> None:
    ...
    try:
        ...
        pool = await asyncpg.create_pool(os.environ.get(
            "DATABASE_URL", "postgresql://algocrypto:algocrypto@localhost:5432/algocrypto"
    ...
    engine = subprocess.Popen(
        [str(venv_python), "-m", "algocrypto.main"],
        cwd=ROOT,
        env={
            **os.environ,
            "PYTHONUNBUFFERED": "1",
    ...
    web = subprocess.Popen(
        [str(venv_python), "-m", "uvicorn", "algocrypto_web.main:app", "--host", "127.0.0.1", "--port", str(WEB_PORT)],
        cwd=ROOT / "web-app",
