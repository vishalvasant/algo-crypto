...
CONFIG_DIR=./config DATABASE_URL="${DATABASE_URL:-postgresql://algocrypto:algocrypto@localhost:5432/algocrypto}" \
  "$VENV" -m algocrypto.main &
ENGINE_PID=$!
...
cd web-app
DATABASE_URL="${DATABASE_URL:-postgresql://algocrypto:algocrypto@localhost:5432/algocrypto}" \
  TRADING_ENGINE_URL="http://127.0.0.1:${ENGINE_PORT}" \
  PYTHONPATH=src \
  "$VENV" -m uvicorn algocrypto_web.main:app --host 0.0.0.0 --port "$WEB_PORT"
