...
ALTER TABLE daily_risk_state
    ADD COLUMN IF NOT EXISTS starting_capital NUMERIC(12, 4) DEFAULT 250,
    ADD COLUMN IF NOT EXISTS available_capital NUMERIC(12, 4) DEFAULT 250,
...
async def ensure_paper_account(capital_inr: float | Decimal = 250) -> None:
    ...
    deployed). Config ``account_capital_usd`` is only the seed when no prior
